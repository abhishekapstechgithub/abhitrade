...
function calcCharges(value: number, productType: string): number {
  ...
  return parseFloat((brokerage + stt + exchangeTxn + gst + sebi + stamp).toFixed(2));
}
...
export const usePaperTradingStore = create<PaperStore>()(
  persist(
    (set, get) => ({
      ...
      placeOrder: (symbol, side, qty, price, orderType = 'MARKET', productType = 'CNC') => {
        const state = get();
        const value = qty * price;
        const charges = calcCharges(value, productType);
        ...
        if (side === 'BUY') {
          if (existingIdx >= 0) {
            ...
            const pnlPercent = parseFloat(((price - newAvg) / newAvg * 100).toFixed(2));
            positions[existingIdx] = { ...ex, quantity: newQty, avgPrice: newAvg, ltp: price, pnl, pnlPercent };
          } else {
            positions.push({ symbol, quantity: qty, avgPrice: price, ltp: price, pnl: 0, pnlPercent: 0, exchange: 'NSE', productType });
          }
        } else {
          if (existingIdx >= 0) {
            const ex = positions[existingIdx];
            ...
            if (newQty <= 0) {
              ...
              const pnlPercent = parseFloat(((price - ex.avgPrice) / ex.avgPrice * 100).toFixed(2));
              positions[existingIdx] = { ...ex, quantity: newQty, ltp: price, pnl, pnlPercent };
            }
          }
        }
      ...
      syncPrices: (priceMap) => {
        ...
        const positions = state.positions.map(pos => {
          ...
          const pnlPercent = parseFloat(((ltp - pos.avgPrice) / pos.avgPrice * 100).toFixed(2));
