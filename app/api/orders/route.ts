...
export async function GET(req: NextRequest) {
  try {
    ...
    const orders = await getOrders(userId, { status, limit, offset, mode });
    return NextResponse.json({ orders, mode });
  } catch (err) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    const order = await createOrder(userId, body, mode);
    return NextResponse.json({ order, mode }, { status: 201 });
  } catch (err) {
    if (err instanceof AuthError) return NextResponse.json({ error: err.message }, { status: 401 });
